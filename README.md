...
- 판정 배지 색상: `불가(빨강) / 가능(난이도 높음, 노랑) / 가능(난이도 낮음, 초록)`
...
- `<DBNAME>_source.html`은 원문 인덱스 페이지이며, `Back to precheck` 링크가 제공됩니다.
- 객체별 페이지에는 `Back to source index`와 `Back to precheck` 링크가 모두 제공됩니다.
- 타입은 `F/P/V`가 아닌 `FUNCTION/PROCEDURE/VIEW` 등 풀네임으로 표기됩니다.
- 키워드가 없으면 `키워드 없음` 배지로 명확히 표시됩니다.
- 객체별 페이지에는 **원문 전체(키워드 색상 강조)** 1개 블록으로 표시되며, 원문을 유지한 채 키워드 텍스트 색상만 강조됩니다.
- `TABLE COLUMN` 항목은 컬럼 단건이 아닌 해당 테이블의 전체 컬럼 원문을 함께 보여줍니다.
