...
Usage: ./epas_precheck_v4.6.sh [options]
Options:
  -h, --host HOST
  -p, --port PORT
...
SYSTEM_SCHEMA_LIST="'pg_catalog', 'information_schema', 'sys', 'dbo', 'sys_catalog', 'enterprisedb', 'htf', 'htp', 'xmltype'"
POLICY_CTE_BODY=""
...
FROM (
    ...
    WHERE n.nspname NOT IN (__SYSTEM_SCHEMA_LIST__)
    ...
    WHERE v.schemaname NOT IN (__SYSTEM_SCHEMA_LIST__)
...
SELECT
    ...
    p.tablename,
    p.policyname,
    COALESCE(row_to_json(p)->>'permissive', '') AS permissive,
    p.roles,
    p.cmd,
    p.qual,
    p.with_check
FROM pg_policies p
WHERE p.schemaname NOT IN (__SYSTEM_SCHEMA_LIST__)
...
WHERE table_schema NOT IN (__SYSTEM_SCHEMA_LIST__)
...
FROM (
    ...
    JOIN pg_namespace n ON c.relnamespace = n.oid
    WHERE con.contype = 'c'
      AND n.nspname NOT IN (__SYSTEM_SCHEMA_LIST__)
...
WITH prof AS (
  ...
  SELECT
    ...
    COALESCE((
      ...
      FROM pg_roles r
      WHERE lower(COALESCE(row_to_json(r)->>'rolprofile','')) = lower(pf.prfname)
         OR lower(COALESCE(row_to_json(r)->>'edb_profile','')) = lower(pf.prfname)
         OR EXISTS (
           SELECT 1
...
WITH prof_users AS (
  SELECT
    ...
    COALESCE((
      SELECT string_agg(r.rolname, ', ' ORDER BY r.rolname)
      FROM pg_roles r
      WHERE lower(COALESCE(row_to_json(r)->>'rolprofile','')) = lower(p.profile)
         OR lower(COALESCE(row_to_json(r)->>'edb_profile','')) = lower(p.profile)
         OR EXISTS (
           SELECT 1
           FROM unnest(COALESCE(r.rolconfig, ARRAY[]::text[])) cfg
...
SELECT
  ...
  COALESCE(
    row_to_json(rg)->>'cpurate',
    row_to_json(rg)->>'rgrpcpurate',
    row_to_json(rg)->>'cpulimit',
    row_to_json(rg)->>'rgrpcpulimit',
    row_to_json(rg)->>'cpu_rate',
    row_to_json(rg)->>'rgrp_cpu_rate',
    (SELECT j.value FROM json_each_text(row_to_json(rg)) j WHERE lower(j.key) LIKE '%cpu%' AND lower(j.key) NOT LIKE '%dirty%' LIMIT 1),
    ''
  ) AS cpurate,
  COALESCE(
    row_to_json(rg)->>'dirtyratelimit',
    row_to_json(rg)->>'rgrpdirtyratelimit',
    row_to_json(rg)->>'dirty_rate_limit',
    row_to_json(rg)->>'rgrp_dirty_rate_limit',
    (SELECT j.value FROM json_each_text(row_to_json(rg)) j WHERE lower(j.key) LIKE '%dirty%' LIMIT 1),
    ''
  ) AS dirtyratelimit,
  COALESCE((
    SELECT string_agg(r.rolname, ', ' ORDER BY r.rolname)
...
SELECT CASE WHEN p.prokind='p' THEN 'P' ELSE 'F' END AS object_type,
       ...
       regexp_replace(
         ...
         'Parallel: ' || CASE COALESCE(row_to_json(p)->>'proparallel', '')
                            WHEN 'r' THEN 'restricted'
                            WHEN 's' THEN 'safe'
                            WHEN 'u' THEN 'unsafe'
                            WHEN '' THEN 'n/a'
                            ELSE COALESCE(row_to_json(p)->>'proparallel', '')
                          END || E'\n' ||
...
JOIN pg_class c ON con.conrelid = c.oid
JOIN pg_namespace n ON c.relnamespace = n.oid
WHERE con.contype = 'c' AND n.nspname NOT IN (__SYSTEM_SCHEMA_LIST__)
UNION ALL
SELECT 'INDEX EXPRESSION' AS object_type, n.nspname AS schema_name, c.relname AS table_name, i.relname AS target_name,
...
JOIN pg_class i ON idx.indexrelid = i.oid
JOIN pg_namespace n ON c.relnamespace = n.oid
WHERE idx.indexprs IS NOT NULL AND n.nspname NOT IN (__SYSTEM_SCHEMA_LIST__);
...
read_sql(){
  ...
  sql="${sql//p.prokind/$PROKIND_EXPR}"
  sql="${sql//__FUNC_SOURCE_EXPR__/$FUNC_SOURCE_EXPR}"
  sql="${sql//__SYSTEM_SCHEMA_LIST__/$SYSTEM_SCHEMA_LIST}"
  sql="${sql//__POLICY_CTE_BODY__/$POLICY_CTE_BODY}"
  printf '%s' "$sql"
}
is_truthy(){
  case "${1,,}" in
    t|true|1|y|yes|on) return 0 ;;
    *) return 1 ;;
  esac
}
...
table_exists(){ is_truthy "$(run_scalar "SELECT to_regclass('$1') IS NOT NULL;")"; }
...
FUNC_SOURCE_EXPR=""
PROKIND_EXPR="p.prokind"
configure_prokind_expr(){
  local has_prokind="f"
  has_prokind="$(run_scalar "SELECT EXISTS (SELECT 1 FROM pg_catalog.pg_attribute WHERE attrelid = 'pg_catalog.pg_proc'::regclass AND attname = 'prokind' AND NOT attisdropped);" 2>/dev/null || true)"
  if is_truthy "$has_prokind"; then
    PROKIND_EXPR="p.prokind"
  else
    PROKIND_EXPR="COALESCE(NULLIF(row_to_json(p)->>'prokind',''),
                      CASE
                        WHEN COALESCE(row_to_json(p)->>'proisagg','f') IN ('t','true','1') THEN 'a'
                        WHEN COALESCE(row_to_json(p)->>'proiswindow','f') IN ('t','true','1') THEN 'w'
                        ELSE 'f'
                      END)"
  fi
}
configure_function_source_expr(){
  ...
  if [[ "$server_version_num" =~ ^[0-9]+$ && "$server_version_num" -ge 140000 ]]; then
    has_sqlbody_fn="$(run_scalar "SELECT (to_regprocedure('pg_catalog.pg_get_function_sqlbody(oid)') IS NOT NULL);" 2>/dev/null || true)"
    if is_truthy "$has_sqlbody_fn"; then
      use_sqlbody=1
    fi
  fi
...
configure_policy_cte_body(){
  if table_exists pg_catalog.pg_policy; then
    POLICY_CTE_BODY=$'  SELECT t.oid,\n         string_agg(\n           format(\'  %s (%s)\', pol.polname,\n             CASE pol.polcmd WHEN \'r\' THEN \'SELECT\' WHEN \'a\' THEN \'INSERT\' WHEN \'w\' THEN \'UPDATE\' WHEN \'d\' THEN \'DELETE\' ELSE pol.polcmd::text END\n           ), E\'\\\\n\' ORDER BY pol.polname\n         ) AS val\n  FROM tbl t\n  JOIN pg_policy pol ON pol.polrelid = t.oid\n  GROUP BY t.oid'
  else
    POLICY_CTE_BODY="  SELECT t.oid, NULL::text AS val FROM tbl t"
  fi
}
configure_prokind_expr
configure_function_source_expr
configure_policy_cte_body
EPAS_VERSION_INFO_RAW="$(run_scalar "SELECT CASE WHEN position(' on ' in split_part(version(), ',', 1)) > 0 THEN split_part(split_part(version(), ',', 1), ' on ', 1) ELSE split_part(version(), ',', 1) END;" 2>/dev/null || true)"
[[ -n "$EPAS_VERSION_INFO_RAW" ]] || EPAS_VERSION_INFO_RAW="Unknown"
EPAS_VERSION_INFO="$(html_escape "$EPAS_VERSION_INFO_RAW")"
OS_NAME_RAW="$(run_scalar "SELECT CASE WHEN version() ~* 'linux' THEN 'Linux' WHEN version() ~* 'windows' THEN 'Windows' WHEN version() ~* '(darwin|mac|os x)' THEN 'macOS' WHEN version() ~* 'freebsd' THEN 'FreeBSD' WHEN version() ~* 'aix' THEN 'AIX' WHEN version() ~* '(sunos|solaris)' THEN 'Solaris' ELSE 'Unknown' END;" 2>/dev/null || true)"
[[ -n "$OS_NAME_RAW" ]] || OS_NAME_RAW="Unknown"
OS_PRETTY_NAME_RAW="$(run_scalar "SELECT CASE WHEN EXISTS (SELECT 1 FROM pg_roles WHERE rolname = current_user AND rolsuper) OR (to_regrole('pg_read_server_files') IS NOT NULL AND pg_has_role(current_user, 'pg_read_server_files', 'member')) THEN COALESCE((regexp_match(pg_read_file('/etc/os-release', 0, 8192), E'(?m)^PRETTY_NAME=\"?([^\"\\n]+)\"?$'))[1], '') ELSE '' END;" 2>/dev/null || true)"
OS_PLATFORM_RAW="$(run_scalar "SELECT CASE WHEN position(' on ' in version()) > 0 THEN split_part(split_part(version(), ' on ', 2), ',', 1) ELSE '' END;" 2>/dev/null || true)"
if [[ -n "$OS_PRETTY_NAME_RAW" ]]; then
  OS_INFO_RAW="$OS_PRETTY_NAME_RAW"
elif [[ -n "$OS_PLATFORM_RAW" ]]; then
  OS_INFO_RAW="${OS_NAME_RAW} (${OS_PLATFORM_RAW})"
else
  OS_INFO_RAW="$OS_NAME_RAW"
fi
...
<div class="card"><h2>Details</h2><p class="sub-link">Source navigator: <a href="${SOURCE_HTML_BASENAME}">${SOURCE_HTML_BASENAME}</a></p><p class="detail-guide">&#49345;&#49464; &#54637;&#47785; &#48516;&#47448; &#47605;</p><div class="details-layout"><div class="detail-shell"><div class="detail-chip-col" id="detail-chips"></div></div><div class="detail-shell"><div class="detail-content" id="detail-content">
