...
Usage: ./epas_precheck_v4.4.sh [options]
Options:
  -h, --host HOST
  -p, --port PORT
...
configure_prokind_expr(){
  ...
  if [[ "$has_prokind" == "t" || "$has_prokind" == "true" || "$has_prokind" == "1" ]]; then
    PROKIND_EXPR="p.prokind"
  else
...
configure_function_source_expr(){
  ...
  if [[ "$server_version_num" =~ ^[0-9]+$ && "$server_version_num" -ge 140000 ]]; then
    ...
    [[ "$has_sqlbody_fn" == "t" || "$has_sqlbody_fn" == "true" || "$has_sqlbody_fn" == "1" ]] && use_sqlbody=1
  fi
