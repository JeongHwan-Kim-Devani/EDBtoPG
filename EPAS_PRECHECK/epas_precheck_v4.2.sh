...
Usage: ./epas_precheck_v4.2.sh [options]
Options:
  -h, --host HOST
  -p, --port PORT
...
configure_prokind_expr(){
  local has_prokind="f"
  has_prokind="$(run_scalar "SELECT EXISTS (SELECT 1 FROM pg_catalog.pg_attribute WHERE attrelid = 'pg_catalog.pg_proc'::regclass AND attname = 'prokind' AND NOT attisdropped)::text;" 2>/dev/null || true)"
  if [[ "$has_prokind" == "t" ]]; then
    PROKIND_EXPR="p.prokind"
  else
    PROKIND_EXPR="CASE WHEN p.proisagg THEN 'a' WHEN p.proiswindow THEN 'w' ELSE 'f' END"
  fi
}
...
function status_label(op){
  return (op=="HIGH"?"?’ìŒ":(op=="MEDIUM"?"ì¤‘ê°„":"??Œ"))
}
function desc_ko(param, raw){
  if(param=="edb_audit") return "EDB ê°ì‚¬ ê¸°ëŠ¥?¼ë¡œ PostgreSQL ?„í™˜ ???€ì²?ê²€? ê? ?„ìš”?©ë‹ˆ??"
  if(param=="edb_audit_archiver") return "EDB ê°ì‚¬ ?„ì¹´?´ë¸Œ ê¸°ëŠ¥?¼ë¡œ PostgreSQL?ì„œ ?™ì¼ ?™ìž‘ ê²€? ê? ?„ìš”?©ë‹ˆ??"
  if(param=="edb_early_lock_release") return "? ê¸ˆ ?´ì œ ?™ìž‘ ì°¨ì´ë¡??¸í•œ ?¸ëžœ??…˜ ?í–¥ ê²€? ê? ?„ìš”?©ë‹ˆ??"
  if(param=="edb_max_capture_privileges_policies") return "ê¶Œí•œ ìº¡ì²˜ ?•ì±… ê´€??EDB ?„ìš© ê¸°ëŠ¥ ?¬ìš© ?¬ë?ë¥??ê??´ì•¼ ?©ë‹ˆ??"
  if(param=="qreplace_function") return "ì¿¼ë¦¬ ?€ì²??¨ìˆ˜ ?¬ìš© ?¬ë? ë°??€ì²?êµ¬í˜„ ê²€? ê? ?„ìš”?©ë‹ˆ??"
  if(param=="edb_stmt_level_tx") return "ë¬¸ìž¥ ?¨ìœ„ ?¸ëžœ??…˜ ì²˜ë¦¬ ì°¨ì´ë¡??¤ë¥˜ ?™ìž‘ ?ê????„ìš”?©ë‹ˆ??"
  if(param=="data_encryption_key_unwrap_command") return "TDE ???´ì œ ëª…ë ¹?€ PostgreSQL ?€ì²?ë°©ì‹ ê²€? ê? ?„ìš”?©ë‹ˆ??"
  if(param=="edb_max_resource_groups") return "ë¦¬ì†Œ??ê·¸ë£¹ ?œí•œ ?¤ì •?€ PostgreSQL ?•ì±…ê³?ë§¤í•‘ ê²€? ê? ?„ìš”?©ë‹ˆ??"
  if(param=="edb_resource_group") return "?¸ì…˜ ë¦¬ì†Œ??ê·¸ë£¹ ?¤ì •???´ì˜ ?•ì±… ?„í™˜ ê²€? ê? ?„ìš”?©ë‹ˆ??"
  if(param=="edb_redwood_strings") return "ë¬¸ìž??NULL ì²˜ë¦¬ ë°©ì‹ ì°¨ì´ë¡?? í”Œë¦¬ì??´ì…˜ ë¡œì§ ê²€? ê? ?„ìš”?©ë‹ˆ??"
  if(param=="db_dialect") return "?¤ë¼???¸í™˜ ë¬¸ë²• ëª¨ë“œê°€ SQL ?™ìž‘???í–¥??ì¤????ˆìŠµ?ˆë‹¤."
  if(param=="datestyle") return "? ì§œ ?Œì‹± ë°?ì¶œë ¥ ?•ì‹ ì°¨ì´ë¡??°ì´??ì²˜ë¦¬ ê²€? ê? ?„ìš”?©ë‹ˆ??"
  if(param=="edb_redwood_greatest_least") return "GREATEST/LEAST??NULL ì²˜ë¦¬ ë°©ì‹ ì°¨ì´ë¥??•ì¸?´ì•¼ ?©ë‹ˆ??"
  if(param=="edb_redwood_date") return "DATE/TIMESTAMP ì²˜ë¦¬ ë°©ì‹ ì°¨ì´ ì¡´ìž¬ ?¬ë?ë¥??•ì¸?´ì•¼ ?©ë‹ˆ??"
  if(param=="edb_dynatune") return "?ë™ ë©”ëª¨ë¦??œë‹ ?„ë¡œ?Œì¼??PostgreSQL ê¸°ë³¸ê°’ê³¼ ?¤ë? ???ˆìŠµ?ˆë‹¤."
  if(param=="edb_dynatune_profile") return "?™ì  ?œë‹ ?„ë¡œ?Œì¼ ê°’ì˜ ?˜ë™ ì¡°ì • ê²€? ê? ?„ìš”?©ë‹ˆ??"
  if(param=="optimizer_mode") return "?µí‹°ë§ˆì´?€ ëª¨ë“œ ?¤ì • ì°¨ì´ë¡??¤í–‰ ê³„íš ê²€? ê? ?„ìš”?©ë‹ˆ??"
  if(param=="default_with_rowids") return "ROWID ?˜ì¡´ SQL?€ PostgreSQL?ì„œ ?¬ìž‘??ê²€? ê? ?„ìš”?©ë‹ˆ??"
  if(param=="enable_hints") return "?ŒíŠ¸ ê¸°ëŠ¥ ?˜ì¡´ ??pg_hint_plan ?ìš© ?¬ë? ê²€? ê? ?„ìš”?©ë‹ˆ??"
  if(param=="oracle_home") return "Oracle DBLink ?°ê³„ ê²½ë¡œ ?•ë³´ ?ê????„ìš”?©ë‹ˆ??"
  if(param=="extension_control_path") return "?•ìž¥ ?œì–´ ê²½ë¡œ ?¤ì •???´ì˜ ?˜ê²½ ë°˜ì˜ ?¬ë?ë¥??•ì¸?´ì•¼ ?©ë‹ˆ??"
  if(param=="edb_redwood_raw_names") return "?€?Œë¬¸???¸ìš© ê°ì²´ëª?ì²˜ë¦¬ ë°©ì‹ ì°¨ì´ë¥??•ì¸?´ì•¼ ?©ë‹ˆ??"
  if(param=="timed_statistics") return "I/O ?€?´ë° ?µê³„ ?˜ì§‘ ?•ì±… ì°¨ì´ë¥??ê??´ì•¼ ?©ë‹ˆ??"
  if(param=="max_generic_plan_partition_size") return "?œë„¤ë¦??Œëžœ ?Œí‹°??ê´€???¤ì • ?í–¥??ê²€? ê? ?„ìš”?©ë‹ˆ??"
  return raw
}
NR>1{
  op="LOW"
