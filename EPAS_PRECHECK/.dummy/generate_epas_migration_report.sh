...
awk -F $'\t' 'NR>1{  op="LOW";  if($1 ~ /^(edb_audit|edb_audit_archiver|edb_early_lock_release|edb_max_capture_privileges_policies|qreplace_function|edb_stmt_level_tx|data_encryption_key_unwrap_command|edb_max_resource_groups|edb_resource_group)$/) op="HIGH";  else if($1 ~ /^(edb_redwood_strings|db_dialect|datestyle|edb_redwood_greatest_least|edb_redwood_date|edb_dynatune|edb_dynatune_profile|optimizer_mode|default_with_rowids|enable_hints)$/) op="MEDIUM";  for(i=1;i<=NF;i++){gsub("&","&amp;",$i);gsub("<","&lt;",$i);gsub(">","&gt;",$i)};  b=(op=="HIGH"?"badge-bad":(op=="MEDIUM"?"badge-high":"badge-low"));  printf "<tr><td><code>%s</code></td><td><code>%s</code></td><td><code>%s</code></td><td>%s</td><td><span class=\"badge %s\">%s</span></td></tr>\n",$1,$2,$3,$5,b,op}' "$OUT_DIR/01_parameters.tsv" > "$PARAM_ROWS"
...
<div class="card"><h2>상세 결과</h2><p class="sub-link">소스 탐색: <a href="${SOURCE_HTML_BASENAME}">${SOURCE_HTML_BASENAME}</a></p>
<h3>1-1. 파라미터 (${param_total})</h3><table><tr><th>파라미터</th><th>기본값</th><th>현재값</th><th>설명</th><th>상태</th></tr>$(default_row_if_empty "$PARAM_ROWS" 5)</table>
<h3>2-1. 호환성 객체 (${feature_total})</h3><table><tr><th>유형</th><th>분류</th><th>객체</th><th>검출 항목</th><th>상태</th></tr>$(default_row_if_empty "$FEATURE_ROWS" 5)</table>
<h3>2-2. 데이터 타입 (${dtype_total})</h3><table><tr><th>유형</th><th>객체</th><th>데이터 타입</th></tr>$(default_row_if_empty "$DTYPE_ROWS" 3)</table>
<h3>2-3. 표현식 (${expr_total})</h3><table><tr><th>객체</th><th>유형</th><th>키워드</th><th>상태</th></tr>$(default_row_if_empty "$EXPR_ROWS" 4)</table>
<h3>2-4. 시노님 (${syn_total})</h3><table><tr><th>시노님</th><th>대상 객체</th><th>상태</th></tr>$( [ -n "$syn_rows_html" ] && echo "$syn_rows_html" || echo '<tr><td colspan="3">데이터 없음</td></tr>' )</table>
<h3>3-1. RLS (${rls_total})</h3><table><tr><th>대상</th><th>정책</th><th>명령</th><th>상태</th></tr>$( [ -n "$rls_rows_html" ] && echo "$rls_rows_html" || echo '<tr><td colspan="4">데이터 없음</td></tr>' )</table>
<h3>3-2. 레드액션 (${redaction_total})</h3><table><tr><th>대상</th><th>정책</th><th>컬럼</th><th>상태</th></tr>$( [ -n "$redaction_rows_html" ] && echo "$redaction_rows_html" || echo '<tr><td colspan="4">데이터 없음</td></tr>' )</table>
<h3>3-3. 프로파일 (${profile_total})</h3><table><tr><th>프로파일</th><th>사용자</th><th>상태</th></tr>$( [ -n "$profile_rows_html" ] && echo "$profile_rows_html" || echo '<tr><td colspan="3">데이터 없음</td></tr>' )</table>
<h3>3-4. 리소스 그룹 (${rg_total})</h3><table><tr><th>그룹</th><th>CPU 비율</th><th>dirtyratelimit</th><th>사용자</th><th>상태</th></tr>$( [ -n "$rg_rows_html" ] && echo "$rg_rows_html" || echo '<tr><td colspan="5">데이터 없음</td></tr>' )</table>
<h3>4-1. DBLINK (${dblink_total})</h3><table><tr><th>DBLINK</th><th>사용자</th><th>연결 정보</th><th>상태</th></tr>$( [ -n "$dblink_rows_html" ] && echo "$dblink_rows_html" || echo '<tr><td colspan="4">데이터 없음</td></tr>' )</table>
</div>
<script>
