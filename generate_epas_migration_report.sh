...
kw={}
for t,s,o,k in rows(out/'03_detail_keywords.tsv'): kw.setdefault(f'{s}.{o}',set()).add(k)
for ot,s,t,tr,k in rows(out/'03_detail_expr_keywords.tsv'): kw.setdefault(f'{s}.{tr}' if ot=='INDEX EXPRESSION' else f'{s}.{t}.{tr}',set()).add(k)
for t,s,o,k in rows(out/'02_summary_packages.tsv'): kw.setdefault(f'{s}.{o}',set()).add(k)
raw={}
for t,s,o,src in rows(out/'02_summary_packages_raw.tsv')+rows(out/'03_detail_keywords_raw.tsv'): raw[f'{s}.{o}']=(t,src)
for ot,s,t,tr,e in rows(out/'03_detail_expr_raw.tsv'): raw[f'{s}.{tr}' if ot=='INDEX EXPRESSION' else f'{s}.{t}.{tr}']=(ot,e)
...
<h3>3-4. 표현식</h3><table><tr><th>객체</th><th>타입</th><th>검출 키워드</th><th>판정</th></tr>$(default_row_if_empty "$EXPR_ROWS" 4)</table>
